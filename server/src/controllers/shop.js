...
module.exports = {
  ...
  async GetShop(req, res) {
    ...
    try {
      ...
      const test = await Shop.findById("5fe8b174e1de0923e8eac98c");
      console.log(test);
  ...
  async ChangeShopStatus(req, res, next) {
    const { id } = req.params;
    ...
    try {
      ...
      if (!shop) {
        return next(
          createHttpError.NotFound(`Request with id [${id}] is not exist`)
        );
      }
      ...
      await User.findByIdAndUpdate(raw.owner, {
        isShopOwner: reqStatus === "approval" ? true : false,
      });

      if (!raw) {
        return next(
          createHttpError.InternalServerError(`Error when edit the request`)
        );
      }
